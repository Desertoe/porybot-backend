...
async function validarEquipoShowdown(paste, regulation) {
    const formatoMap = {
    ...
    try {
        ...
        const response = await fetch('https://play.pokemonshowdown.com/~~showdown/action.php?act=validateteam', {
            method: 'POST',
...
async function createTeam(ownerId, userRole, data) {
    // Límite de equipos para usuarios básicos
    if (userRole === 'user') {
        const [existing] = await database_1.pool.query('SELECT COUNT(*) as count FROM teams WHERE owner_id = ? AND type = ?', [ownerId, data.type]);
    ...
    for (const poke of parsedPokemon) {
        await database_1.pool.query('INSERT INTO team_pokemon (id, team_id, slot, species, item, ability, tera_type) VALUES (UUID(), ?, ?, ?, ?, ?, ?)', [teamId, poke.slot, poke.species, poke.item, poke.ability, poke.tera_type]);
    }
    return getTeamById(teamId, ownerId);
}
async function getTeamById(teamId, userId) {
    const [rows] = await database_1.pool.query('SELECT * FROM teams WHERE id = ?', [teamId]);
    if (rows.length === 0)
        throw new Error('Team not found');
    const team = rows[0];
    const [pokemon] = await database_1.pool.query('SELECT * FROM team_pokemon WHERE team_id = ? ORDER BY slot', [teamId]);
    const [[lk]] = await database_1.pool.query('SELECT COUNT(*) as likes_count FROM team_likes WHERE team_id = ?', [teamId]);
    const [[sv]] = await database_1.pool.query('SELECT COUNT(*) as saves_count FROM team_saves WHERE team_id = ?', [teamId]);
    let user_liked = false, user_saved = false;
    if (userId) {
        const [lr] = await database_1.pool.query('SELECT 1 FROM team_likes WHERE user_id = ? AND team_id = ?', [userId, teamId]);
        user_liked = lr.length > 0;
        const [sr] = await database_1.pool.query('SELECT 1 FROM team_saves WHERE user_id = ? AND team_id = ?', [userId, teamId]);
        user_saved = sr.length > 0;
    }
    return { ...team, pokemon, likes_count: lk.likes_count, saves_count: sv.saves_count, user_liked, user_saved };
}
async function getUserTeams(ownerId, filters) {
...
async function updateTeam(teamId, ownerId, data) {
    const fields = [];
    const values = [];
    ...
    if (data.regulation !== undefined) {
        ...
        values.push(data.regulation);
    }
    if (data.paste !== undefined) {
        fields.push('paste = ?');
        values.push(data.paste);
    }
    if (fields.length === 0)
        throw new Error('Nothing to update');
    values.push(teamId, ownerId);
    await database_1.pool.query('UPDATE teams SET ' + fields.join(', ') + ' WHERE id = ? AND owner_id = ?', values);
    if (data.paste) {
        await database_1.pool.query('DELETE FROM team_pokemon WHERE team_id = ?', [teamId]);
...
async function deleteTeam(teamId, ownerId) {
    const [result] = await database_1.pool.query('DELETE FROM teams WHERE id = ? AND owner_id = ?', [teamId, ownerId]);
